...
{   
    ...
    {
        case APP_MOUNT_DISK:
            if(SYS_FS_Mount("/dev/mmcblkb1", "/mnt/SD_LOGG", FAT, 0, NULL) != 0)
            {
        ...
        case APP_WRITE_MEASURE_FILE:
            appFatData.fileHandle = SYS_FS_FileOpen("logger.csv",
...
void sd_BNO_scheduleWrite (s_bno055_data * data)
{
    ...
    if(appFatData.state == APP_IDLE)
    {
        ...
        // Write the buffer //
        sprintf(appFatData.data, "%d;%d0;%f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%d;%d;%d;%d;\r\n"
                                  ,data->flagImportantMeas, (data->d_time), data->pressure, data->gravity.x, data->gravity.y, data->gravity.z, data->gyro.x, data->gyro.y, data->gyro.z
...
*/
void sd_logger_scheduleWrite (mcp79411_time *data)
{
    // If sd Card available 
    if(appFatData.state == APP_IDLE)
    {
        ...
        appFatData.state = APP_WRITE_MEASURE_FILE;
        // Write the buffer //
        sprintf(appFatData.data, "%d_%d_%d,",data->min,data->hour,data->wkday);
        //Compute the number of bytes to send 
        appFatData.nBytesToWrite = strlen(appFatData.data);
    }
}
